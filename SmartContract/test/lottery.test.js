...
describe("Lottery", () => {
  ...
  context("Basic Check", async () => {
    ...
    it("Check if parameters are stored well through constructor", async () => {
      ...
      expect(await lottery.startingBlock()).to.eq(LOTTERY_STARTING_BLOCK);
  ...
  context("announce", async () => {
    ...
    before("Block passes for announcement", async () => {
    ...
    it("Announce", async () => {
      /// WINNING_NUMBER will be generated by VRF on OZ's defender but it is simply tested with no VRF
      ...
      /// This function will be executed in OZ's defender
      await expect(
        lottery.announce(
          WINNING_NUMBER,
          totalFirstPlaceWinners,
    ...
    it("Check if the information of the round is correctly updated", async () => {
      ...
      expect(onlyFirstPlacePrizeAmountFromTeam).to.eq(
        roundInfo.onlyFirstPlacePrizeAmountFromTeam
      );
