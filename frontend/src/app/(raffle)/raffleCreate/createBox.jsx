...
export default function CreateBox() {
  ...
  const onChangePrizeAddress = useCallback(
    async (e) => {
      ...
      setInputValue({
        ...
        prizeAmount: ref.prizeType === 1 ? 1 : inputValue.prizeAmount,
        prizeId: ref.prizeType === 0 ? 0 : "",
      });
    },
    [inputValue]
  );
  ...
  const onChangePrizeAmount = useCallback(
    async (e) => {
      if (ref.prizeType === 1) {
        return toastMessage("Must be 1 for ERC721", "warn");
      }
  ...
  const create = useCallback(
    async (e) => {
      ...
      const raffle = new Contract(raffleJson.address, raffleJson.abi, signer);
      ...
      if (depositAmountOfFusdt > fusdtBalance)
        return toastMessage(
          `Insufficient deposit amount ${formattedBalance(
            depositAmountOfFusdt,
            6
          )} FUSDT`,
          "warn"
        );
      const allowance = await fusdtInstance.allowance(
        address,
        raffleJson.address
      );
  ...
  return (
    <form
      onSubmit={create}
      className="font-bebas_neue grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 card gap-5 xl:gap-10"
    >
      <div className="flex flex-col gap-2">
        <label id="address">Current Prize Type</label>
        <div className="raffleHighlightBox">{inputValue.prizeTypeText}</div>
      </div>
      ...
      <div className="flex flex-col  gap-2  ">
        <label>CREATOR</label>
        <div className="raffleHighlightBox">
